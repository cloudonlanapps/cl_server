...
uv run pytest services/store/tests --auth-url=http://${SERVER_IP}:8010 --compute-url=http://${SERVER_IP}:8012 --username=admin --password=admin --mqtt-url=mqtt://${SERVER_IP}:1883 "${EXTRA_ARGS[@]}"
